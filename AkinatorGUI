...
public class Akinator {
    ...
    public static void main(String[] args) {
       ...
       JOptionPane.showMessageDialog(null,"\tPense em um cantor...\n\tAgora responda às perguntas para que possamos adivinhá-lo!", "Bem vindo ao Akinator!",JOptionPane.INFORMATION_MESSAGE);
        ...
        for (int i = 0; i < perguntas.length; i++) {
           ...
           respostas[i]= respostas[i].toLowerCase();
        }
        ...
        if (cantoresFiltrados.length == 1) {
           JOptionPane.showMessageDialog(null,"O cantor que você está pensando é: " + cantoresFiltrados[0].nome + "!!");
           int opcao = JOptionPane.showConfirmDialog(null, "Deseja jogar novamente?","Reiniciar Quiz",JOptionPane.YES_NO_OPTION);
           
           if (opcao != JOptionPane.YES_OPTION) {
                reiniciar = false;
            }
        } else {
            int opcao = JOptionPane.showConfirmDialog(null, "Poxa...não consegui advinhar seu cantor, gostaria de tentar novamente?","Reiniciar",JOptionPane.YES_NO_OPTION);
